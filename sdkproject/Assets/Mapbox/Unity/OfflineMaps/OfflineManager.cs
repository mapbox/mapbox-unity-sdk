...
namespace Mapbox.Unity.OfflineMaps
{
	public class OfflineManager
	{
		...
		{
			var swWebMerc = new Vector2d(Math.Max(min.x, -Utils.Constants.WebMercMax), Math.Max(min.y, -Utils.Constants.WebMercMax));
			var neWebMerc = new Vector2d(Math.Min(max.x, Utils.Constants.WebMercMax), Math.Min(max.y, Utils.Constants.WebMercMax));
