...
namespace Mapbox.Unity.MeshGeneration.Modifiers
{
	...
	public class LineMeshForPolygonsModifier : MeshModifier
	{
		...
		private static bool IsOnEdge(Vector3 p1, Vector3 p2, float tolerance)
		{
			return ((Math.Abs(Math.Abs(p1.x) - 50) < tolerance && Math.Abs(Math.Abs(p2.x) - 50) < tolerance && Math.Sign(p1.x) == Math.Sign(p2.x)) ||
			        (Math.Abs(Math.Abs(p1.z) - 50) < tolerance && Math.Abs(Math.Abs(p2.z) - 50) < tolerance && Math.Sign(p1.z) == Math.Sign(p2.z)));
		}
