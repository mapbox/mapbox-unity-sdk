...
namespace Mapbox.MapboxSdkCs.UnitTest
{
	...
	internal class GeocoderTest
	{
...
#else
		private readonly Geocoder _geocoder = MapboxAccess.Instance.Geocoder;
#endif
		...
		public IEnumerator Forward()
		{
			...
			{
				ForwardGeocodeResource forwardResource = new ForwardGeocodeResource("vienna, at");
				ForwardGeocodeResponse forwardResponse = await _geocoder.GeocodeForward(forwardResource);

				Assert.NotNull(forwardResponse.Features, "'GeocodeForward(vienna, at)' did not return any results");
				Assert.Greater(forwardResponse.Features.Count, 0, "'GeocodeForward(vienna, at)' did not return any results");
				Assert.IsTrue(
					forwardResponse.Features.Any(f =>
						(f.PlaceName.Contains("Vienna") || f.PlaceName.Contains("Wien"))
						&& (f.PlaceName.Contains("Austria") || f.PlaceName.Contains("Österreich"))
					)
					, "'GeocodeForward(vienna, at)' did not properly geocode to Vienna, Austria"
				);
				running = false;
			};
			asyncWorkaround();
		...
		public IEnumerator Reverse()
		{
			...
			Action asyncWorkaround = async () =>
			{
				Vector2d saintStephansCathedral = new Vector2d(48.20848, 16.37313);
				ReverseGeocodeResource reverseResource = new ReverseGeocodeResource(saintStephansCathedral);
				ReverseGeocodeResponse reverseResponse = await _geocoder.GeocodeReverse(reverseResource);

				Assert.NotNull(reverseResponse.Features, "'GeocodeReverse(48.20848, 16.37313)' did not return any results");
				Assert.Greater(reverseResponse.Features.Count, 0, "'GeocodeReverse(48.20848, 16.37313)' did not return any results");
				Assert.IsTrue(
					reverseResponse.Features.Any(f =>
						(f.PlaceName.Contains("Vienna") || f.PlaceName.Contains("Wien"))
						&& (f.PlaceName.Contains("Austria") || f.PlaceName.Contains("Österreich"))
						&& f.PlaceName.Contains("1010")
					)
					, "'GeocodeReverse(48.20848, 16.37313)' did not properly geocode to Vienna, Austria, 1st district"
				);
