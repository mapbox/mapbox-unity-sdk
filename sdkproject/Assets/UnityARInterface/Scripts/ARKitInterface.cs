...
namespace UnityARInterface
{
	public class ARKitInterface : ARInterface
	{
		...
		private CustomTrackingState _trackingState;
		...
		void UpdateFrame(UnityARCamera camera)
		{
			...
			//Custom Here. Getting TrackingQuality.
			ConvertTrackingState(camera);
		}

		void ConvertTrackingState(UnityARCamera Camera)
		{
			if (Camera.trackingState == ARTrackingState.ARTrackingStateNormal)
			{
				_trackingState = CustomTrackingState.Good;
			}
			if (Camera.trackingState == ARTrackingState.ARTrackingStateLimited)
			{
				_trackingState = CustomTrackingState.Paused;
			}
			if (Camera.trackingState == ARTrackingState.ARTrackingStateNotAvailable)
			{
				_trackingState = CustomTrackingState.Stopped;
			}
		}
		...
		public override bool GetTrackingState(ref CustomTrackingState trackingState)
		{
			...
			{
				case CustomTrackingState.Paused:
					trackingState = CustomTrackingState.Paused;
					return true;
				case CustomTrackingState.Good:
					trackingState = CustomTrackingState.Good;
					return true;
				case CustomTrackingState.Stopped:
					trackingState = CustomTrackingState.Stopped;
					return true;
				default:
					return false;
			}
